...
#pragma omp prallel for
            for (size_t i = 0; i < count; ++i)  mem_out[i] = std::exp(mem_in[i]);
...
template <>
struct OperSeq_<OpTags::Exp>
{
    using type = OperCalAlgoChain<TailCalculator<OperExp::NSCaseGen::EvalItem, OperExp::NSCaseGen::EvalGroup>>;
};

template <typename TP,
          std::enable_if_t<IsValidOper<OpTags::Exp, TP>>* = nullptr>
auto Exp(TP&& p_m)
{
    using rawM = RemConstRef<TP>;
    using ResType = Operation<OpTags::Exp, OperandContainer<rawM>>;
...
namespace MetaNN
{
namespace OperExpGrad::NSCaseGen
{
    ...
    protected:
        virtual void EvalInternalLogic(EvalItemType& evalItem) final override
        {
            ...
            {
                mem_out[i] = mem_grad[i % grad_count] * std::exp(mem_in[i]);
            }
            evalItem.m_outputHandle.SetData(std::move(out));
        }
    };
...
template <>
struct OperSeq_<OpTags::ExpGrad>
{
    using type = OperCalAlgoChain<TailCalculator<OperExpGrad::NSCaseGen::EvalItem, OperExpGrad::NSCaseGen::EvalGroup>>;
};

template <typename TGrad, typename TInput,
          std::enable_if_t<IsValidOper<OpTags::ExpGrad, TGrad, TInput>>* = nullptr>
auto ExpGrad(TGrad&& p_grad, TInput&& p_input)
{
